...
<script language="javascript" type="text/javascript">
	...
	function checkMQSelect() {
		...
		if (infoId.length == 0) {
			...
			return;
		} else if (infoId.length > 1) {
			ymPrompt.alert("一次只能选择一条实例!");
			return;
		} else {
			location.href = "getInstanceDetial?serId=" + infoId + "&ptype=mq";
		}
	}
														...
														success : function(
															...
															for (var i = 0; i < jsonstr.length; i++) {
																...
																for ( var key in jsonstr[i]) {
																	if (key == '_id') {
																		ht = ht
																				+ '<td style="text-align: center;"><input type="checkbox" name="servers" value="'
																				+ jsonstr[i][key]
																				+ '" onclick="isSelect(this);" /></td>';
																	} else if (key == 'Status') {
																		if (jsonstr[i][key] == 'Active')
																			ht = ht
																					+ '<td style="text-align: center;" name="state"><img src="img/icon_success.png"></img>&nbsp;&nbsp;<b>Active</b></td>';

																		if (jsonstr[i][key] == 'Error')
																			ht = ht
																					+ '<td style="text-align: center;" name="state"><img src="img/icon_error.png"></img>&nbsp;&nbsp;<b>Error</b></td>';
																	} else if (key == 'edit') {
																		ht = ht
																				+ '<td style="text-align: center;"><img src="img/edit.png" id="postsubmit" name="postsubmit"   onmousedown="old=this.src; this.src="img/edit18.png";onmouseup="this.src=old;" onclick="editHost(this);"/></td>';
																	} else {
																		ht = ht
															...
															$('table tbody')
																	.html(ht);
															//$('#tableid').attr('class','table table-bordered data-table  with-check table-hover no-search no-select');
...
<body>
	...
	<div class="content">
		<div class="breadcrumb">
			...
			<a href="#" title="IBM MQ" class="tip-bottom">IBM MQ</a>
		</div>
				...
				<div class="span12">
					<div class="widget-box collapsible">
						...
						<div id="collapseOne" class="collapse in">
							<div class="widget-content">所有MQ实例信息.</div>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="row-fluid">
				<div class="span12">
						...
						<div class="widget-box nostyle">
							<%-- 			<div class="col-sm-6 form-inline">
								<input class="form-control" id="search" type="text"
									placeholder="请输入主机名或IP地址" style="width: 160px" /> <img
									src="<%=path%>/img/icon-search.png" id="postsubmit"
									name="postsubmit"
					...
					<div class="columnfoot">
						<a class="btn btn-info fr btn-next" onclick="checkMQSelect();">
